...
double findProductPrice(struct Shop s, char* n){
	...
	for (int i = 0; i < s.index; i++){
		...
		if (strcmp(name, n) == 0 ){
			//printf("This item %s\n", name);
			//int qtyInShop = findProductQty(shop, name);
			return pd.price;	
		}
	}
		printf("...SORRY, this shop does not have any item called '%s' \n", n);
		return 0;
}
...
int findProductQty(struct Shop s, char* n){
	for (int i = 0; i < s.index; i++){
		...
		// else {
// 			printf("...SORRY, Product out of stock");
// 			//return -1;
// 		}
	}
		//printf("...SORRY, Product out of stock");
...
// Find Product
// struct Product validateProduct(struct Shop s, char* n){
// 	for (int i = 0; i < s.index; i++){
// 		struct Product pd = s.stock[i].product;
// 		char *name = pd.name;
//
//
// 		if (strcmp(name, n) == 0 ){
// 			printf("Product exists in shop: %s\n", name);
// 			return pd;
// 		}
// 		// else {
// // 			printf("...SORRY, Product out of stock");
	...
	double itemPrice;
		for( cindx = 0; cindx < (sizeof( custItems ) / sizeof( custItems[0] )); cindx++){
			if (custItems[cindx] != NULL){
				...
				//printf("PrPrice is %3df\n", qtyInShop);
				//printf("PrQTY available: %3d\n", qtyInShop);
